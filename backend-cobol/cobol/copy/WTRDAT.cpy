      ...
      *>   固定長 97 byte / ネイティブ(ASCII表示数字 + COMP-3 + Shift-JISテキスト)。
      *>   ※ 旧 EBCDIC/存10進/JEF は廃止。数字は PIC 9 表示(ASCII), 金額は
      *>     COMP-3(純COBOLパック, JEFサービス不要), 摘要は Shift-JIS(2026-08、
      *>     Instant Client Basic 化で NLS_LANG が実際に効くようになったため)。
      *>   TR-EXT は 種別で再定義(REDEFINES):
