...
--
--  ------------------------------------------------------------
--  ★このファイルについて (tools/parity/handoff-test/)
--    backend-cobol/sql/90_parity_fixture.sql の複製。値は一切変更していない
--    (=正解側)。対になる java_fixture.sql は意図的に1箇所だけ値を変えてある。
--    第三者に渡すCOBOL→Java比較ツールの検証用セット。
--  ------------------------------------------------------------
--
--  実行: sqlplus -s /nolog @cobol_fixture.sql
-- ============================================================
...
--      4001213 普通  1件(区分3)                 利息0 + 手数料合計は0ではない
